<?php
   ...
   if($_SESSION['username']==""){
    ...
    if(isset($_POST['save_order'])){
      ...
      $total = $_POST['total'];
      $customer_bal = $_POST['customerbalance'];
      $paid = $_POST['paid'];
      $due = $_POST['due'];
      ...
      if($arr_product_code != ""){
        ...
        $insert = $pdo->prepare("INSERT INTO tbl_invoice(customer_id, customer_name, customer_add, cashier_name, order_date, time_order, total, customer_bal,  paid, due)
        values(:customerid, :customername, :customeraddress, :name, :orderdate, :timeorder, :total, :customerbalance, :paid, :due)");

        $insert->bindParam(':customerid',  $customer_id);
        $insert->bindParam(':customername',  $customer_name);
        $insert->bindParam(':customeraddress',  $customer_add);
        $insert->bindParam(':name', $cashier_name);
        $insert->bindParam(':orderdate',  $order_date);
        $insert->bindParam(':timeorder',  $order_time);
        $insert->bindParam(':total', $total);        
        $insert->bindParam(':customerbalance',  $customer_bal);
...
?>
  ...
  <div class="content-wrapper">
    ...
    <div class="box box-danger">
          ...
          <table class="table table-border" id="myCus">
                  <thead>
                      <tr>
                          <th>Customer Name</th>
                          <th>Address</th>
                          <th>Balance</th>
                          <th>
                            <button type="button" name="addCustomer" class="btn btn-success btn-sm btn_addCustomer" required><span>
        ...
        <div class="box box-success">
          <form action="" method="POST">
            ...
            <div class="box-footer" align="center">
              <input type="submit" name="save_order" value="Save Sales" class="btn btn-success">
  ...
  //Date picker
    ...
    $(document).ready(function(){
      $(document).on('click','.btn_addCustomer', function(){
        var html='';
        html+='<tr>';
        html+='<td><input type="hidden" class="form-control customername" name="customername" readonly></td>';
        html+='<td><select class="form-control customerid" name="customerid" style="width:200px;" required><option value="">--Select Customer--</option><?php
        echo fill_customer($pdo)?></select></td>';
        html+='<td><input type="text" class="form-control customeraddress" style="width:200px;" name="customeraddress" readonly></td>';
        html+='<td><input type="number" class="form-control customerbalance" style="width:200px;" name="customerbalance" readonly></td>';
        html+='<td><button type="button" name="remove" class="btn btn-danger btn-sm btn-remove"><i class="fa fa-remove"></i></button></td>'
        ...
        $('.customerid').on('change', function(e){
          ...
          $.ajax({
            ...
            success:function(data){
              //console.log(data);
              tr.find(".customername").val(data["name"]);
              tr.find(".customeraddress").val(data["address"]);
              tr.find(".customerbalance").val(data["balance"]);
            }
          })
        })
